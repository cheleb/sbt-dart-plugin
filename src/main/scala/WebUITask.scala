...
trait WebUITask extends DartKeys {
  ...
  lazy val webuicTask = (dartDirectory in Compile, dartWebUIEntryPoints, resourceManaged in Compile, dartOptions) map { (dartDir, inputs, resources, options) =>
    println("WebUI Task")
    DartCompiler.compileWebUI(dartDir, options)
